            ...
            <div key={categoryName} className="team-category-section">
              ...
              {adminMode ? (
                <ReorderableGrid
                  items={members}
                  admin={adminMode}
                  containerClass="card-grid team-grid"
                  itemClass="card team-card glass-card"
                  onReorder={(newGroupMembers) => {
                    const updatedTeam = team.map((tm) => {
                      const match = newGroupMembers.find((item) => item.id === tm.id)
                      return match ? { ...tm, sortOrder: match.sortOrder } : tm
                    })
                    reorderRecords('team', updatedTeam, setTeam)
                  }}
                  renderItem={(member) => (
                    <>
